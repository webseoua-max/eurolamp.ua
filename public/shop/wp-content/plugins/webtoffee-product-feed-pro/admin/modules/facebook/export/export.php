...
if (!class_exists('Webtoffee_Product_Feed_Sync_Pro_Facebook_Export')) {
    ...
    class Webtoffee_Product_Feed_Sync_Pro_Facebook_Export extends Webtoffee_Product_Feed_Pro_Product {
        ...
        public function prepare_data_to_export($form_data, $batch_offset, $step) {
            ...
            if ($batch_offset < $export_limit) {
                ...
                if (!empty($prod_types)) {
                    ...
                    if( count($prod_types) == 1 && 'variable' !== $prod_types[0]){
                        $include_variation = false;
                    }
                }

                if ( '' === $include_variations_type && $include_variation ) {
                    array_push($args['type'], 'variation');
                }   
                
                if ( isset($args['type']) && in_array('variation', $args['type'])) {
                    array_push($args['type'], 'variable');
                }                
                
                if( '' !== $wt_pf_export_post_author ){
                    $args['author'] = is_scalar( $wt_pf_export_post_author ) ? $wt_pf_export_post_author : implode(',', $wt_pf_export_post_author);
                }
                ...
                foreach ($products_ids as $key => $product_id) {
                    ...
                    //Skip variation other than lowest when Only include lowest product variation is checked.
                    if ($product->is_type('variation') && 'lowest' === $include_variations_type ) {
                        continue;
                    }                    

                    if ($product->is_type('variable') && 'default' === $include_variations_type ) {
                        $default_variation_id = $this->get_default_variation($product);
                        $use_as_variable_product = apply_filters('wt_product_feed_keep_default_as_variation', true);
                        if ($default_variation_id && $use_as_variable_product ) {
                            $product = wc_get_product($default_variation_id);
                        }
                        if(!$default_variation_id){
                            $include_variations_type = 'lowest';
                        }                        
                    }
        ...
        protected function generate_row_data_wc_lower($product_object) {
            ...
            foreach ($export_columns as $key => $value) {
                if (method_exists($this, $value)) {
                    ...
                    $product_for_attr = $product_object;
                    if($product_object->is_type('variation')){
                        $product_for_attr = wc_get_product($product_object->get_parent_id());
                    }
                    $attr_value = '';
                    if(is_object($product_for_attr)){
                        $attr_value = $product_for_attr->get_attribute( $atr_key );
                    }
                    if ( ! empty( $attr_value ) ) {
        ...
        public function id($catalog_attr, $product_attr, $export_columns) {
            ...
            $id = apply_filters('wt_feed_filter_product_id', $fb_retailer_id, $this->product);
        ...
        public function parent_title($catalog_attr, $product_attr, $export_columns) {
            ...
            return apply_filters("wt_feed_{$this->parent_module->module_base}_parent_title", $title, $this->product, $this->form_data);
        }
        ...
        public function condition($catalog_attr, $product_attr, $export_columns) {
            ...
            $condition = ( $custom_condition) ? $custom_condition : 'new';
        ...
        public function fb_product_category($catalog_attr, $product_attr, $export_columns) {
            ...
            if ( !$custom_fb_category) {
                ...
                if(is_string($custom_fb_category) || is_numeric($custom_fb_category)){
                $fb_category_list = wp_cache_get('wt_fbfeed_fb_product_categories_array');
                ...
                $fb_product_category = isset($fb_category_list[$custom_fb_category]) ? $fb_category_list[$custom_fb_category] : '';
                }else{
                    $fb_product_category = '';
                }
            }
        ...
        public function availability($catalog_attr, $product_attr, $export_columns) {
            ...
            if( isset( $this->form_data['post_type_form_data']['wt_pf_include_variations_type'] ) && '' !==  $this->form_data['post_type_form_data']['wt_pf_include_variations_type'] && 'out of stock' === $status && $this->product->is_type('variation') ){
                // If default or lowest priced variation is selected and if it's out of stock handle here
                $status = 'in stock';
            }
        ...
        public function price_with_tax($catalog_attr, $product_attr, $export_columns) {
            ...
            $price = apply_filters('wt_feed_filter_product_price_with_tax', $price, $this->product);
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_price_with_tax", $price, $this->product, $this->form_data);
        }
        ...
        public function current_price_with_tax($catalog_attr, $product_attr, $export_columns) {
            ...
            $price = apply_filters('wt_feed_filter_product_current_price_with_tax', $price, $this->product);
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_current_price_with_tax", $price, $this->product, $this->form_data);
        }
        ...
        public function sale_price_with_tax($catalog_attr, $product_attr, $export_columns) {
                ...
                $min_max_discounted_price = $product_sale->get_min_max_discounted_price();
                    if(is_array($min_max_discounted_price)){    
                        ...
                        if ( !empty( $discounted_prices ) ) {
                            $dprice = $discounted_prices[0];
                            $price = \wc_get_price_including_tax( $this->product, array( 'price' => $dprice ) );
                        }
                }
            }           
            ...
            $price = apply_filters('wt_feed_filter_product_sale_price_with_tax', $price, $this->product);
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_sale_price_with_tax", $price, $this->product, $this->form_data);
        }
        ...
        public function first_variation_price() {
            ...
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_first_variation_price", $price, $this->product, $this->form_data);
        }
        ...
        public function weight($catalog_attr, $product_attr, $export_columns) {
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_weight", $this->product->get_weight(), $this->product, $this->form_data);
        }
        ...
        public function weight_unit($catalog_attr, $product_attr, $export_columns) {
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_weight_unit", get_option('woocommerce_weight_unit'), $this->product, $this->form_data);
        }
        ...
        public function width($catalog_attr, $product_attr, $export_columns) {
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_width", $this->product->get_width(), $this->product, $this->form_data);
        }
        ...
        public function height($catalog_attr, $product_attr, $export_columns) {
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_height", $this->product->get_height(), $this->product, $this->form_data);
        }
        ...
        public function length($catalog_attr, $product_attr, $export_columns) {
            return apply_filters("wt_feed_{$this->parent_module->module_base}_product_length", $this->product->get_length(), $this->product, $this->form_data);
        }
        ...
        public function fb_override($catalog_attr, $product_attr, $export_columns) {
            $override_country = $this->form_data['post_type_form_data']['wt_pf_export_catalog_country'];
