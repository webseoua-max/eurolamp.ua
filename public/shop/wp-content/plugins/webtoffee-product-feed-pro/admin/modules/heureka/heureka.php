 ...
 * @link          
 *
 * @package  Webtoffee_Product_Feed_Sync_Pro_Heureka
 */
if (!defined('ABSPATH')) {
	exit;
}

if (!class_exists('Webtoffee_Product_Feed_Sync_Pro_Heureka')) {

	class Webtoffee_Product_Feed_Sync_Pro_Heureka {

		public $module_id = '';
		public static $module_id_static = '';
		public $module_base = 'heureka';
		public $module_name = 'Webtoffee Product Feed Catlaog for Heureka';
	...
	?>
		...
		<tr class="form-field">
			<th scope="row" valign="top"><label for="wt_google_category"><?php esc_html_e('Google Category', 'webtoffee-product-feed-pro'); ?></label></th>
			<td>
                <select name="wt_google_category" class="wc-enhanced-select">
					<?php echo wp_kses_post(wt_google_category_dropdown($fb_category_id)); ?>
				</select>
	...
	if ( isset( $_POST[ 'wt_google_category' ] ) ) {
		...
		public function exporter_do_export($export_data, $base, $step, $form_data, $selected_template_data, $method_export, $batch_offset) {
			...
			$export = new Webtoffee_Product_Feed_Sync_Pro_Heureka_Export($this);
		...
		public function wt_pf_exporter_post_types_basic($arr) {
			...
			$arr['heureka'] = __('Heureka', 'webtoffee-product-feed-pro');
			return $arr;
		}
		...
		public function exporter_alter_meta_mapping_fields($fields, $base, $step_page_form_data) {
			...
			return $fields;
                ...
                public function product_attributes_dropdown($attribute_dropdown, $export_channel, $selected=''){
                    ...
                    if( 'heureka' === $export_channel ){
                        
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'name', 'PRODUCT' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'image_link_alternate', 'IMGURL_ALTERNATIVE' );
                        //$attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'video_url', 'VIDEO_URL' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'ean', 'EAN' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'price_with_tax', 'PRICE_VAT' );
                        
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'brand', 'MANUFACTURER' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'category', 'CATEGORYTEXT' );                        
...
new Webtoffee_Product_Feed_Sync_Pro_Heureka();
