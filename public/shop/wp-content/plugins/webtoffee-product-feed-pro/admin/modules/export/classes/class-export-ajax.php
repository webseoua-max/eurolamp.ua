...
class Webtoffee_Product_Feed_Sync_Pro_Export_Ajax
{
		...
		if($this->to_export!='')
		{
			...
			if($template_data)
			{
				...
				if($is_throw_warn)
				{
					...
					if($step=='save_as')
					{
						$out['msg']=__('Please enter a different name', 'webtoffee-product-feed-pro');
					}else
					{
						$out['msg']=__('Template with same name already exists', 'webtoffee-product-feed-pro');	
					}
					return $out;
				}		
			}			
			...
			{ 
				...
				if($wpdb->update($tb, $update_data, $update_where, $update_data_type, $update_where_type)!==false)
				{
					...
					$out['msg']=__('Template updated successfully', 'webtoffee-product-feed-pro');
					$out['name']=$template_name;
					return $out;
				}
			}else
			{
				...
				if($wpdb->insert($tb, $insert_data, $insert_data_type)) //success
				{
					...
					$out['msg']=__('Template saved successfully', 'webtoffee-product-feed-pro');
					return $out;
				}
			}
		}
	...
	*/
	public function post_type_page()
	{
		$post_types=apply_filters('wt_pf_exporter_post_types_basic', array());
                $feed_page_heading = isset( $_REQUEST['rerun_id'] ) ? __('Edit feed', 'webtoffee-product-feed-pro') : __('Create new feed', 'webtoffee-product-feed-pro');
		...
		if ($this->rerun_id > 0) {
                    ...
                    $item_country = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_catalog_country']) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_export_catalog_country'] : '';                    
                    ...
                    $item_author = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_post_author']) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_export_post_author'] : '';
                    ...
                    if (isset($this->export_obj->form_data['post_type_form_data']['tag_filter_type'])) {
                        $item_tag_filter_type = isset($this->export_obj->form_data['post_type_form_data']['tag_filter_type']) ? $this->export_obj->form_data['post_type_form_data']['tag_filter_type'] : 'include_tag';
                    } elseif (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_tag_filter_type'])) {
                        $item_tag_filter_type = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_tag_filter_type']) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_export_tag_filter_type'] : 'include_tag';
                    }                    
                    
                    if (isset($this->export_obj->form_data['post_type_form_data']['inc_exc_tag'])) {
                        $inc_exc_tag = isset($this->export_obj->form_data['post_type_form_data']['inc_exc_tag']) ? $this->export_obj->form_data['post_type_form_data']['inc_exc_tag'] : array();
                    } elseif (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_inc_exc_tag'])) {
                        $inc_exc_tag = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_inc_exc_tag']) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_inc_exc_tag'] : array();
                    }                    

                    if (isset($this->export_obj->form_data['post_type_form_data']['brand_filter_type'])) {
                        $item_brand_filter_type = isset($this->export_obj->form_data['post_type_form_data']['brand_filter_type']) ? $this->export_obj->form_data['post_type_form_data']['brand_filter_type'] : 'include_brand';
                    } elseif (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_brand_filter_type'])) {
                        $item_brand_filter_type = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_brand_filter_type']) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_export_brand_filter_type'] : 'include_brand';
                    }

                    if (isset($this->export_obj->form_data['post_type_form_data']['inc_exc_brand'])) {
                        $inc_exc_brand = isset($this->export_obj->form_data['post_type_form_data']['inc_exc_brand']) ? $this->export_obj->form_data['post_type_form_data']['inc_exc_brand'] : array();
                    } elseif (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_inc_exc_brand'])) {
                        $inc_exc_brand = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_inc_exc_brand']) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_inc_exc_brand'] : array();
                    }
                    ...
                    if (isset($this->export_obj->form_data['post_type_form_data']['item_outofstock'])) {
                        $item_outofstock = ( isset($this->export_obj->form_data['post_type_form_data']['item_outofstock']) && ( $this->export_obj->form_data['post_type_form_data']['item_outofstock'] == 'on' || $this->export_obj->form_data['post_type_form_data']['item_outofstock'] == 1 ) ) ? 1 : 0;
                    } elseif (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_exclude_outofstock'])) {
                        $item_outofstock = ( isset($this->export_obj->form_data['post_type_form_data']['wt_pf_exclude_outofstock'] ) && ( $this->export_obj->form_data['post_type_form_data']['wt_pf_exclude_outofstock'] == 'on' || $this->export_obj->form_data['post_type_form_data']['wt_pf_exclude_outofstock'] == 1 ) ) ? 1 : 0;
                    }

                    if (isset($this->export_obj->form_data['post_type_form_data']['item_parentonly'])) {
                        $item_inc_parentonly = ( isset($this->export_obj->form_data['post_type_form_data']['item_parentonly'] ) && ( $this->export_obj->form_data['post_type_form_data']['item_parentonly'] == 'on' || $this->export_obj->form_data['post_type_form_data']['item_parentonly'] == 1 ) ) ? 1 : 0;
                    } elseif (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_include_parent_only'])) {
                        $item_inc_parentonly = ( isset($this->export_obj->form_data['post_type_form_data']['wt_pf_include_parent_only'] ) && ($this->export_obj->form_data['post_type_form_data']['wt_pf_include_parent_only'] == 'on' || $this->export_obj->form_data['post_type_form_data']['wt_pf_include_parent_only'] == 1) ) ? 1 : 0;
                    }
                    if($item_inc_parentonly){
                        $variations_type_selected = 'default';
                    }else{
                        $variations_type_selected = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_include_variations_type'] ) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_include_variations_type'] : '';
                    }
                    ...
                    } elseif (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_catalog_cron_type'])) {
                        $item_gen_cron_type = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_export_catalog_cron_type']) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_export_catalog_cron_type'] : 'wordpress_cron';
                    }
                    ...
                    if (isset($this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_if[]'])) {

                        $wt_pf_adv_filter_fields = !empty($this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_if[]']) ? (array) $this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_if[]'] : array();
                        $wt_pf_adv_filter_condition = !empty($this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_condition[]']) ? (array) $this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_condition[]'] : array();
                        $wt_pf_adv_filter_val = !empty($this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_val[]']) ? (array) $this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_val[]'] : array();
                        $wt_pf_adv_filter_then = !empty($this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_then[]']) ? (array) $this->export_obj->form_data['post_type_form_data']['wt_pf_adv_filter_then[]'] : array();
                    ...
                    $wt_pf_parent_qty = isset($this->export_obj->form_data['post_type_form_data']['wt_pf_parent_qty'] ) ? $this->export_obj->form_data['post_type_form_data']['wt_pf_parent_qty'] : '';
                }
	...
	*/
	public function method_export_page()
	{
		...
		if($this->to_export!="")
		{
			...
			return Wt_Pf_IE_Basic_Helper::sanitize_and_minify_html($html_data);
		}else
		{
			return '';
		}
	...
	*/
	...
	{	
                 ...
                 */
                $skip_category_mapping_channels = array( 'google_local_product_inventory', 'google_promotions', 'buyon_google', 'pricerunner', 'skroutz', 'idealo', 'heureka', 'pinterest_rss', 'yandex', 'vivino', 'google_manufacturer', 'shopmania' );
		...
		if($current_index!==false) /* step exists */
		{
			if($current_index>0) //add back button
			{
				$out['back']=array(
					...
					'text'=>'<span class="dashicons dashicons-arrow-left-alt2" style="line-height:27px;"></span> '.__('Back', 'webtoffee-product-feed-pro'),
				);
			}
			...
			if(isset($step_keys[$current_index+1])) /* not last step */
			{
				...
				$out['next']=array(
					...
					'action_type'=>'step',
					'key'=>$next_key,
					'text'=>__('Step', 'webtoffee-product-feed-pro').' '.$next_number.': '.$next_title.' <span class="dashicons dashicons-arrow-right-alt2" style="line-height:27px;"></span>',
				);
				...
				if($this->export_method=='quick' || $this->export_method=='template') //Quick Or Template method
				{
					$out['or']=array(
						'type'=>'text',
						'text'=>__('Or', 'webtoffee-product-feed-pro'),
					);
				}
