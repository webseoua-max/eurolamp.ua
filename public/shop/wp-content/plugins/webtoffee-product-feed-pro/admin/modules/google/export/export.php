...
if (!class_exists('Webtoffee_Product_Feed_Sync_Pro_Google_Export')) {

    class Webtoffee_Product_Feed_Sync_Pro_Google_Export extends Webtoffee_Product_Feed_Pro_Product {
        ...
        protected function generate_row_data_wc_lower($product_object) {
            ...
            foreach ($export_columns as $key => $value) {
                if (method_exists($this, $value)) {                    
                    ...
                    $value = '';
                    if($product_object->is_type('variation')){
                        $value = $this->custom_attr_data($product_object, $atr_key);
                        $product_obj = wc_get_product($product_object->get_parent_id());
                    }
                    if( '' === $value && is_object($product_obj)){
                        $value = $product_obj->get_attribute( $atr_key );
                    }
                    if ( ! empty( $value ) ) {
				    $value = trim( $value );
			}
