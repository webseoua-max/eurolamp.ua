...
<div class="wt_productfeed_schedule_now wt_productfeed_popup">
	<div class="wt_productfeed_popup_hd">
		...
		<span class="wt_productfeed_popup_hd_label"><?php esc_html_e('Update Schedule', 'webtoffee-product-feed-pro');?></span>
...
<!--	<ul class="wt_productfeed_sub_tab">
	...
	<div class="wt_productfeed_schedule_now_box">
		<div class="wt_productfeed_cron_current_time"><b><?php esc_html_e('Current server time:', 'webtoffee-product-feed-pro');?></b> <span>--:--:-- --</span><br/>
		
			<?php 
			$wt_time_zone = Webtoffee_Product_Feed_Sync_Pro_Common_Helper::get_advanced_settings('default_time_zone'); 
			if(!$wt_time_zone){
			?>
			<data><?php esc_html_e('To switch to your website timezone', 'webtoffee-product-feed-pro'); ?> <a href="<?php echo esc_url(admin_url('admin.php?page=wt_import_export_for_woo'));?>" target="_blank" style="text-decoration:none;"> <?php esc_html_e('click here', 'webtoffee-product-feed-pro' ); ?> <em class="dashicons dashicons-external"></em></a></data>
			<?php } ?>
		
		</div>

		<label><?php esc_html_e('Schedule type', 'webtoffee-product-feed-pro');?></label>
		<div class="wt_productfeed_schedule_now_formrow">
			...
			<div class="wt_productfeed_schedule_type_box_single" style="margin-bottom:0px;">
                <label for="wt_productfeed_schedule_wordpress_cron"><input type="radio" name="wt_productfeed_schedule_type" id="wt_productfeed_schedule_wordpress_cron" value="wordpress_cron" <?php checked('wordpress_cron', $cron_data['schedule_type'], true); ?> > <?php esc_html_e('Wordpress Cron', 'webtoffee-product-feed-pro');?> </label>
				<p class="wt_productfeed_schedule_type_desc"><?php esc_html_e('This type of scheduler depends on the Wordpress for scheduling your job at the specified time. However this model is dependent on your website visitors. Upon a visit Wordpress cron will check to see if the time/date is later than the scheduled event/s, and if it is– it will fire those events.', 'webtoffee-product-feed-pro');?></p>
			</div>
			<div class="wt_productfeed_schedule_type_box_single">
				<label for="wt_productfeed_schedule_server_cron"><input type="radio" name="wt_productfeed_schedule_type" id="wt_productfeed_schedule_server_cron" value="server_cron" <?php checked('server_cron', $cron_data['schedule_type'], true); ?>> <?php esc_html_e('Server Cron', 'webtoffee-product-feed-pro');?> </label>
				<p class="wt_productfeed_schedule_type_desc">
					<?php esc_html_e('You can use this option if you have a separate system to trigger the scheduled events. This method will generate a unique URL which can be added to your system inorder to trigger the events. You may need to trigger the URL every minute depending on the volume of data to be processed.', 'webtoffee-product-feed-pro');?>					
				</p>
			</div>
		</div>
		...
		{
		?>
		<label><?php esc_html_e('File name', 'webtoffee-product-feed-pro');?></label>
		<div class="wt_productfeed_schedule_now_formrow">
			...
			<br />	
			<?php esc_html_e('Specify a filename for the exported file(the contents of this file will be overwritten for every export). If left blank the system generates a default name(a new filename is generated for every export).', 'webtoffee-product-feed-pro'); ?>	
		</div>
		<?php
		}
		?>

		<label><?php esc_html_e('Interval', 'webtoffee-product-feed-pro');?></label>
		<div class="wt_productfeed_schedule_now_formrow">			
			<div class="wt_productfeed_schedule_now_interval_radio_block">
				<label for="wt_productfeed_cron_interval_day"><input type="radio" id="wt_productfeed_cron_interval_day" name="wt_productfeed_cron_interval" value="day" <?php checked('day', $cron_data['interval'], true); ?>> <?php esc_html_e('Every day', 'webtoffee-product-feed-pro');?></label>
				<label for="wt_productfeed_cron_interval_week"><input type="radio" id="wt_productfeed_cron_interval_week" name="wt_productfeed_cron_interval" value="week" <?php checked('week', $cron_data['interval'], true); ?>> <?php esc_html_e('Every week', 'webtoffee-product-feed-pro');?></label>
				<!-- <label for="wt_productfeed_cron_interval_biweek"><input type="radio" id="wt_productfeed_cron_interval_biweek" name="wt_productfeed_cron_interval" value="biweek"> <?php esc_html_e('Biweekly', 'webtoffee-product-feed-pro');?></label> -->
				<label for="wt_productfeed_cron_interval_month"><input type="radio" id="wt_productfeed_cron_interval_month" name="wt_productfeed_cron_interval" value="month" <?php checked('month', $cron_data['interval'], true); ?>> <?php esc_html_e('Every month', 'webtoffee-product-feed-pro');?></label>
				<label for="wt_productfeed_cron_interval_custom"><input type="radio" id="wt_productfeed_cron_interval_custom" name="wt_productfeed_cron_interval" value="custom" <?php checked('custom', $cron_data['interval'], true); ?>> <?php esc_html_e('Custom', 'webtoffee-product-feed-pro');?></label>
			</div>
			<div class="wt_productfeed_schedule_now_interval_sub_block wt_productfeed_schedule_custom_interval_block">
				<label><?php esc_html_e('Custom interval', 'webtoffee-product-feed-pro');?></label>
				<input type="number" step="1" min="1" name="wt_productfeed_cron_interval_val" value="<?php echo esc_attr($cron_data['custom_interval']); ?>" placeholder="<?php esc_attr_e('Interval in minutes.', 'webtoffee-product-feed-pro');?>">
				<span class="wt-iew_form_help" style="margin-top:3px;"><?php esc_html_e('Recommended: Minimum 2 hour(120 minutes)', 'webtoffee-product-feed-pro');?></span>
			</div>
			<div class="wt_productfeed_schedule_now_interval_sub_block wt_productfeed_schedule_day_block">
				<label><?php esc_html_e('Which day?', 'webtoffee-product-feed-pro');?></label>
				<div class="wt_productfeed_schedule_now_interval_radio_block">				
					<?php
					...
					foreach ($days as $day)
					{
						...
						?>
						<label for="wt_productfeed_cron_day_<?php echo esc_attr($day_vl);?>"><input type="radio" value="<?php echo esc_attr($day_vl);?>" id="wt_productfeed_cron_day_<?php echo esc_attr($day_vl);?>" name="wt_productfeed_cron_day" <?php echo $checked;?>> <?php esc_html_e($day, 'webtoffee-product-feed-pro');?></label>
						<?php
					}
					?>
				</div>
			...
			<div class="wt_productfeed_schedule_now_interval_sub_block wt_productfeed_schedule_date_block">
				<label><?php esc_html_e('Day of the Month?', 'webtoffee-product-feed-pro');?></label>
				<select name="wt_productfeed_cron_interval_date">
					...
					?>
					<option value="last_day"><?php esc_html_e('Last day', 'webtoffee-product-feed-pro');?></option>
				</select>
			</div>
                    <?php
			...
			<div class="wt_productfeed_schedule_now_interval_sub_block wt_productfeed_schedule_starttime_block">
				<label><?php esc_html_e('Start time', 'webtoffee-product-feed-pro');?></label> 
		...
		<div class="wt_productfeed_schedule_now_trigger_url">
			<label><?php esc_html_e('Trigger URL', 'webtoffee-product-feed-pro');?></label>
			<div class="wt_productfeed_schedule_now_formrow" style="margin-bottom:0px;">
				<input type="text" name="wt_productfeed_cron_url" value="" />
				<p style="color:red; margin:0px;"><?php esc_html_e('Use the generated URL to run cron.', 'webtoffee-product-feed-pro'); ?></p>
...
<!--			<div class="wt_productfeed_sub_tab_content" data-id="cron-file-details">
		...
		<div class="wt_productfeed_popup_footer">
			<button type="button" name="" class="button-secondary wt_productfeed_popup_cancel">
				<?php esc_html_e('Cancel', 'webtoffee-product-feed-pro');?> 
			</button>
			<button type="button" name="" class="button-primary wt_productfeed_update_schedule" style="margin-right:20px;"><?php esc_html_e('Update Schedule', 'webtoffee-product-feed-pro');?></button>	
		</div>
