...
if (!class_exists('Webtoffee_Product_Feed_Sync_Pro_Fruugo_Export')) {

    class Webtoffee_Product_Feed_Sync_Pro_Fruugo_Export extends Webtoffee_Product_Feed_Pro_Product {
        ...
        protected function generate_row_data_wc_lower($product_object) {
            ...
            if (isset($this->form_data['advanced_form_data']['wt_pf_file_as']) && 'xml' === $this->form_data['advanced_form_data']['wt_pf_file_as']) {
                ...
                unset($export_columns['title'], $export_columns['language'], $export_columns['currency'], $export_columns['price_without_vat'], $export_columns['vat_rate'] , $export_columns['AttributeColor'], $export_columns['AttributeSize'] );
                $export_columns['Price'] = 'Price';
                $export_columns['Description'] = 'Description';
                if($this->max_variation_count > 0 ){
                   for($i=1; $i<=$this->max_variation_count; $i++){
                       unset($export_columns['Attribute'.$i]);
                   }  
            ...
            foreach ($export_columns as $key => $value) {               
				...
				$value = trim( $value );
                    ...
                    if ('(+)' === $mode) {                        
                        ...
                        // Special handling for price_without_vat field - use the same field name as XML
                        if ('price_without_vat' === $key) {
                            $row[$key] = $this->wt_pf_product_field_calc('price_without_tax', $amount, 'increase', $product_object);
                        } else {
                    ...
                    if ('(-)' === $mode) {
                        ...
                        // Special handling for price_without_vat field - use the same field name as XML
                        if ('price_without_vat' === $key) {
                            $row[$key] = $this->wt_pf_product_field_calc('price_without_tax', $amount, 'decrease', $product_object);
                        } else {
                    ...
                    if ('(*)' === $mode) {                        
                        ...
                        // Special handling for price_without_vat field - use the same field name as XML
                        if ('price_without_vat' === $key) {
                            $row[$key] = $this->wt_pf_product_field_calc('price_without_tax', $amount, 'multiply', $product_object);
                        } else {
	...
	public function isbn($catalog_attr, $product_attr, $export_columns){
        ...
        public function description($catalog_attr, $product_attr, $export_columns) {
           ...
           if (isset($this->form_data['advanced_form_data']['wt_pf_file_as']) && 'xml' === $this->form_data['advanced_form_data']['wt_pf_file_as']) {
                ...
                $description = array(
                   ...
                   'Title' => $this->wt_remove_emoji( $this->title($catalog_attr, $product_attr, $export_columns) ),
        ...
        public function get_shipping() {
            ...
            $shipping_obj = new Webtoffee_Product_Feed_Shipping($this->product, 'fruugo', $this->form_data);
                ...
                foreach ($category_path as $category) {
                    ...
                    if ($tiktok_category_id) {
                        ...
                        if (false === $tiktok_category_list) {
                            $tiktok_category_list = Webtoffee_Product_Feed_Sync_Pro_Fruugo::get_category_array();
        ...
        public function Category($catalog_attr, $product_attr, $export_columns) {
                ...
                foreach ($category_path as $category) {
                    ...
                    if ($fruugo_category_id) {
                        ...
                        if (false === $fruugo_category_list) {
                            $fruugo_category_list = Webtoffee_Product_Feed_Sync_Pro_Fruugo::get_category_array();
            ...
            if ( !empty( $fruugo_product_category ) ) {
                ...
                $fruugo_product_category = is_array($fruugo_product_category) ? $fruugo_product_category[0] : '';
            }            
            
            return apply_filters('wt_feed_filter_product_fruugo_categories', $fruugo_product_category, $this->product);            
        ...
        public function AttributeHandler($catalog_attr, $product_attr, $export_columns, $attr_count) {
            return $this->wt_variation_option($catalog_attr, $product_attr, $export_columns, $attr_count);
        }
        ...
        public function price($catalog_attr, $product_attr, $export_columns) {
            ...
            if (isset($this->form_data['advanced_form_data']['wt_pf_file_as']) && 'xml' === $this->form_data['advanced_form_data']['wt_pf_file_as']) {
                $price = array(
                 ...
                 if( strpos($this->fruugo_xml_skipfeed_keys['price_without_vat'], 'wt_compute_map_vl:') !== false) {
                     $price['NormalPriceWithoutVAT'] =  $this->compute_field_mapping($this->fruugo_xml_skipfeed_keys['price_without_vat'], 'price_without_tax', $this->product);
                 }
