 ...
 * @link          
 *
 * @package  Webtoffee_Product_Feed_Sync_Fruugo
 */
if (!defined('ABSPATH')) {
	exit;
}

if (!class_exists('Webtoffee_Product_Feed_Sync_Pro_Fruugo')) {

	class Webtoffee_Product_Feed_Sync_Pro_Fruugo {

		public $module_id = '';
		public static $module_id_static = '';
		public $module_base = 'fruugo';
		public $module_name = 'Webtoffee Product Feed Catlaog for Fruugo';
		...
		public function __construct() {
			...
			add_filter('wt_pf_feed_category_mapping', array($this, 'map_fruugo_category'), 10, 1);
...
public function wt_fbfeed_category_form_fields_pro( $category ) {
	...
	?>
		...
		<tr class="form-field">
			...
			<th scope="row" valign="top"><label for="wt_fruugo_category"><?php esc_html_e('Fruugo Category', 'webtoffee-product-feed-pro');?></label></th>
			<td>
                            <select name="wt_fruugo_category" class="wc-enhanced-select">
		<?php echo wt_fruugo_category_dropdown( $fruugo_category_id ); ?>
				</select>

				<p class="description"><?php esc_html_e('The Fruugo Category corresponding to this category in the website.', 'webtoffee-product-feed-pro')?>
				</p>
			</td>
		</tr>
	...
	public function wt_fbfeed_category_form_save_pro( $term_id ) {
            ...
            if ( isset( $_POST[ 'wt_fruugo_category' ] ) ) {
		if(! wp_verify_nonce( $_POST['wt_category_edit_nonce'], 'wt_category_edit_nonce' )){
			return false;
		}

		$wt_fruugo_category = absint( $_POST[ 'wt_fruugo_category' ] );
		if(0 == $wt_fruugo_category){
			delete_term_meta($term_id, 'wt_fruugo_category');
		}else{
			update_term_meta( $term_id, 'wt_fruugo_category', $wt_fruugo_category );
		}
	}
}         
        public function wt_fbfeed_category_form_save_pro_ajx_new( $term_id, $tt_id, $update, $args ) {

            if ( isset( $args[ 'wt_fruugo_category' ] ) ) {
                        if(! wp_verify_nonce( $args['wt_category_edit_nonce'], 'wt_category_edit_nonce' )){
                                return false;
                        }

                        $wt_google_category = absint( $args[ 'wt_fruugo_category' ] );
                        if(0 == $wt_google_category){
                                delete_term_meta($term_id, 'wt_fruugo_category');
                        }else{
                                update_term_meta( $term_id, 'wt_fruugo_category', $wt_google_category );
                        }
                }
        ...
        public function wt_fbfeed_category_form_save_pro_ajx( $term_id, $tt_id, $update ) {           
            ...
            if ( isset( $_POST[ 'wt_fruugo_category' ] ) ) {
                        ...
                        $wt_google_category = absint( $_POST[ 'wt_fruugo_category' ] );
                        if(0 == $wt_google_category){
                                delete_term_meta($term_id, 'wt_fruugo_category');
                        }else{
                                update_term_meta( $term_id, 'wt_fruugo_category', $wt_google_category );
		...
		public function map_fruugo_category($form_data) {
			...
			if ( ( isset($form_data['post_type_form_data']['item_type']) &&  $form_data['post_type_form_data']['item_type'] != $this->module_base ) || ( isset($form_data['post_type_form_data']['wt_pf_export_post_type']) &&  $form_data['post_type_form_data']['wt_pf_export_post_type'] != $this->module_base )) {
				...
				foreach ($form_data['category_mapping_form_data'] as $local_cat => $merchant_cat) {
					if (!empty($merchant_cat)) {
						...
						$wt_fruugo_category = absint($merchant_cat);
						update_term_meta($term_id, 'wt_fruugo_category', $wt_fruugo_category);
					}
				}
				return $form_data;
		...
		public function exporter_do_export($export_data, $base, $step, $form_data, $selected_template_data, $method_export, $batch_offset) {
			...
			$export = new Webtoffee_Product_Feed_Sync_Pro_Fruugo_Export($this);
		...
		public function wt_pf_exporter_post_types_basic($arr) {
			...
			$arr['fruugo'] = __('Fruugo', 'webtoffee-product-feed-pro');
			return $arr;
		}
		...
		public static function get_category_array() {
			// Get All Fruugo Taxonomies
					
			$taxonomy = wp_cache_get('wt_iew_feed_fruugo_categories');
			...
			if ($customTaxonomyFile) {
				...
				wp_cache_set('wt_iew_feed_fruugo_categories', $taxonomy, '', WEEK_IN_SECONDS);
			}
		...
		public function exporter_alter_mapping_enabled_fields($mapping_enabled_fields, $base, $form_data_mapping_enabled_fields) {
			if ($base == $this->module_base) {
				...
				$mapping_enabled_fields['product_details'] = array(__('Product Details', 'webtoffee-product-feed-pro'), 1);																																								
			}
		...
		public function exporter_alter_meta_mapping_fields($fields, $base, $step_page_form_data) {
                        ...
                        foreach ($fields as $key => $value) {
                                switch ($key) {
                                    case 'product_details':
                                        $fields[$key]['fields']['Imageurl1'] = 'Imageurl1';
                                        $fields[$key]['fields']['Imageurl2'] = 'Imageurl2';
                                        $fields[$key]['fields']['Imageurl3'] = 'Imageurl3';
                                        $fields[$key]['fields']['language'] = 'Language';                                        
                                        $fields[$key]['fields']['AttributeColor'] = 'AttributeColor';
                                        $fields[$key]['fields']['AttributeSize'] = 'AttributeSize';                                        
                                        break;
                            ...
                            if( $max_variations > 0 ){                            
                                for($i=1; $i<=$max_variations; $i++){
                                    $fields['product_details']['fields']['Attribute'.$i] = 'Attribute'.$i;                                
                                }
                            }
			...
			return $fields;
                ...
                public function product_attributes_dropdown($attribute_dropdown, $export_channel, $selected=''){
                    ...
                    if( 'fruugo' === $export_channel ){
                        
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'currency', 'Currency' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'price_without_vat', 'NormalPriceWithoutVAT' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'vat_rate', 'VATRate' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'EAN', 'EAN' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'isbn', 'ISBN' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'availability', 'StockStatus' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'package_weight', 'PackageWeight' );
                        
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'Imageurl1', 'Imageurl1' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'Imageurl2', 'Imageurl2' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'Imageurl3', 'Imageurl3' );
                        
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'language', 'Language' );
                        
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'AttributeColor', 'AttributeColor' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'AttributeSize', 'AttributeSize' );
                        ...
                        if( $max_variations > 0 ){  
                            ...
                            for($i=1; $i<=$max_variations; $i++){
                                $select_val = 'Attribute'.$i;
                                $select_key = 'Attribute'.$i;
			...
			foreach ($fields as $fieldk => $fieldv) {
				$out[$fieldk] = $fieldv;
                        ...
                        if ('fruugo' === $base) {
                            ...
                            $out['delimiter']['help_text']= __( 'Separator for differentiating the columns in the CSV file. Assumes comma by default.', 'webtoffee-product-feed-pro' );
                        }
                        return $out;
		}
		...
		public function exporter_alter_filter_fields($fields, $base, $filter_form_data) {
			...
			return $fields;
                ...
                public static function wt_feed_get_product_conditions() {
                        ...
                        return apply_filters( 'wt_feed_fruugo_product_conditions', $conditions );
                }	

                public static function get_age_group() {
                                $fruugo_age_group = array(
                                ...
                                return apply_filters( 'wt_feed_fruugo_product_agegroup', $fruugo_age_group );
...
new Webtoffee_Product_Feed_Sync_Pro_Fruugo();


// FB Category dropdown caching
			if (!function_exists('wt_fruugo_category_dropdown')) {

				function wt_fruugo_category_dropdown($selected = '') {

					$category_dropdown = wp_cache_get('wt_fruugofeed_dropdown_product_categories');

					if (false === $category_dropdown) {
						$categories = Webtoffee_Product_Feed_Sync_Pro_Fruugo::get_category_array();
						...
						wp_cache_set('wt_fruugofeed_dropdown_product_categories', $category_dropdown, '', WEEK_IN_SECONDS);
					}
