 ...
 * @link           
 *
 * @package  Webtoffee_Product_Feed_Sync_Pro_History 
 */
if (!defined('ABSPATH')) {
    exit;
}
...
class Webtoffee_Product_Feed_Sync_Pro_History
{
	...
	public function wt_product_feed_pro_load_translations_history() {
		self::$status_label_arr=array(
			0=>__('Running/Incomplete', 'webtoffee-product-feed-pro'),
			1=>__('Finished', 'webtoffee-product-feed-pro'),
			2=>__('Failed', 'webtoffee-product-feed-pro'),
		);

		self::$action_label_arr=array(
			'export'=>__('Export', 'webtoffee-product-feed-pro'),
			'import'=>__('Import', 'webtoffee-product-feed-pro'),
			'export_image'=>__('Image Export', 'webtoffee-product-feed-pro'),
		);
	}
	...
	*/
	public function add_admin_pages($menus)
	{
		...
		$menus[$this->module_base]=array(
			...
			__('Manage Feeds', 'webtoffee-product-feed-pro'),
			__('Manage Feeds', 'webtoffee-product-feed-pro'),
	...
	*/ 
	public function advanced_setting_fields($fields)
	{
		...
		$fields['advanced_field_head'] =array(
				...
				'head'=>__('Advanced options', 'webtoffee-product-feed-pro'),
				'group_id'=>'advanced_field', //field group id
				'show_on_default'=>0,
			);
		$fields['enable_history_auto_delete']=array(
			'label'=>__("Auto delete history", 'webtoffee-product-feed-pro'),
			'type'=>'checkbox',
			'checkbox_fields' => array( 1 => __( 'Enable', 'webtoffee-product-feed-pro' ) ),
			'value' => 1,
			'field_name'=>'enable_history_auto_delete',
			'field_group'=>'advanced_field',
			'help_text'=>__('Enable auto delete for records within the history section.', 'webtoffee-product-feed-pro'),
		...
		$fields['auto_delete_history_count']=array(
			'label'=>__("Maximum entries", 'webtoffee-product-feed-pro'),
			'type'=>'number',
            'value' =>100,
            'attr' =>array(
            	'style'=>'width:30%;',
	...
	*/
	public function admin_settings_page($args)
	{
		...
		*/ 
		$filter_by=array(
			'item_type'=>array(
				'label'=>__('Post type', 'webtoffee-product-feed-pro'),
			...
			'template_type'=>array(
				'label'=>__('Action type', 'webtoffee-product-feed-pro'),
			...
			'status'=>array(
				'label'=>__('Status', 'webtoffee-product-feed-pro'),
				'values'=>$status_arr,
		...
		*/
		$order_by=array(
			'date_desc'=>array('label'=>__('Date descending', 'webtoffee-product-feed-pro'), 'sql'=>'created_at DESC'),
			'date_asc'=>array('label'=>__('Date ascending', 'webtoffee-product-feed-pro'), 'sql'=>'created_at ASC'),
		);
	...
	*/
	...
	public static function update_fbcron_entry($history_id, $update_data, $update_data_type)
	{
		global $wpdb;
		//updating the data
		...
		{ 
			if(Wt_Pf_Sh::check_write_access(WEBTOFFEE_PRODUCT_FEED_PRO_ID)) /* check nonce and role */
			{
				...
				if($file_name!="")
				{
					...
					{
						$file_path= '';//Webtoffee_Product_Feed_Sync_Pro_Log::get_file_path($file_name);
    ...
    */
	public static function gen_pagination_html($total, $limit, $offset, $url, $url_params=array(), $mxnav=6)
	{
		...
		$html='<span class="wt_pf_pagination_total_info">'.$total.__(' record(s)', 'webtoffee-product-feed-pro').'</span>';
