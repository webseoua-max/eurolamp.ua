    ...
    function wt_onbuy_feed_render_categories( $parent = 0, $par = '', $value = '' ) {
		...
		if(!empty($par))
            ...
            ?>
                <tr class="treegrid-1">
                        <td>
                                <?php esc_html_e('All categories have already been mapped', 'webtoffee-product-feed-pro'); ?>
                        </td>
                </tr>
            <?php
            }
...
<div class="wt-wrap">
	...
	<?php
        ...
        <span><?php esc_html_e( 'OnBuy has a pre-defined set of categories', 'webtoffee-product-feed-pro'); ?></a>. <?php esc_html_e( 'Mapping your store categories with the OnBuy categories will give more visibility to your products in OnBuy ads and listings. To edit the mapping go to the respective', 'webtoffee-product-feed-pro'); ?> <a target="_blank" href="<?php echo esc_url(admin_url('edit-tags.php?taxonomy=product_cat&post_type=product')); ?>"><?php esc_html_e( 'categories page', 'webtoffee-product-feed-pro'); ?></a></span>
	<?php else: ?>
        <span><?php echo esc_html($feed_channel_name); if( 'tiktok' === $this->to_export ){ $feed_channel_name = 'TikTok';} // To avoid ads text multiple times ?> <?php esc_html_e( 'uses', 'webtoffee-product-feed-pro'); ?> <a target="_blank" href="https://www.onbuy.com/basepages/producttype/taxonomy.en-US.txt"><?php esc_html_e( 'OnBuy categories', 'webtoffee-product-feed-pro'); ?></a>. <?php esc_html_e( 'Mapping your store categories with the OnBuy categories will give more visibility to your products in', 'webtoffee-product-feed-pro');?> <?php echo esc_html($feed_channel_name); ?> <?php esc_html_e( 'ads. To edit the mapping go to the respective', 'webtoffee-product-feed-pro'); ?> <a target="_blank" href="<?php echo esc_url(admin_url('edit-tags.php?taxonomy=product_cat&post_type=product')); ?>"><?php esc_html_e( 'categories page', 'webtoffee-product-feed-pro'); ?></a></span>
        <?php endif; ?>
