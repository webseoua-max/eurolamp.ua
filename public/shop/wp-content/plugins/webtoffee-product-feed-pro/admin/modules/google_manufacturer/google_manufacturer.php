 ...
 * @link          
 *
 * @package  Webtoffee_Product_Feed_Sync_Pro_Google_Manufacturer 
 */
if (!defined('ABSPATH')) {
	exit;
}

if (!class_exists('Webtoffee_Product_Feed_Sync_Pro_Google_Manufacturer')) {

	class Webtoffee_Product_Feed_Sync_Pro_Google_Manufacturer {

		public $module_id = '';
		public static $module_id_static = '';
		public $module_base = 'google_manufacturer';
		public $module_name = 'Webtoffee Product Feed Catlaog for Google Manufacturer';
		...
		public function exporter_do_export($export_data, $base, $step, $form_data, $selected_template_data, $method_export, $batch_offset) {
			...
			$export = new Webtoffee_Product_Feed_Sync_Pro_Google_Manufacturer_Export($this);
		...
		public function fn_wt_pf_exporter_steps_basic($steps, $to_export) {
			...
			if ('google_manufacturer' === $to_export) {
			...
			return $steps;
                ...
                public function product_attributes_dropdown($attribute_dropdown, $export_channel, $selected = '') {
                    ...
                    if ('google_manufacturer' === $export_channel) {

                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'disclosure_date', 'disclosure_date[disclosure_date]' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'release_date', 'release_date[release_date]' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'suggested_retail_price', 'suggested_retail_price[suggested_retail_price]' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'product_line', 'product_line[product_line]' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'size_type', 'size_type' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'size_system', 'size_system' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'product_detail', 'product_detail' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'feature_description', 'feature_description' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'video_link', 'video_link' );
                        $attribute_dropdown .= sprintf( '<option value="%s">%s</option>', 'product_page_url', 'Product URL[link]' );
		...
		public function wt_pf_exporter_post_types_basic($arr) {
			...
			$arr['google_manufacturer'] = __('Google Manufacturer Center', 'webtoffee-product-feed-pro');
			return $arr;
		}
...
new Webtoffee_Product_Feed_Sync_Pro_Google_Manufacturer();
			...
			if (!function_exists('wt_google_category_dropdown')) {
				...
				function wt_google_category_dropdown($selected = '') {
					...
					if (false === $category_dropdown) {
						$categories = Webtoffee_Product_Feed_Sync_Pro_Google_Manufacturer::get_category_array();
