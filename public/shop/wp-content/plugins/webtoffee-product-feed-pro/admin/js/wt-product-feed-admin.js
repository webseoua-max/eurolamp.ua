	...
	{ 
		...
		wf_prnt_obj.find('.wt_pf_settings_form_basic').submit(function(e){
			...
			jQuery.ajax({
				...
				type:'POST',
				dataType:'json',
				data:data+'&action=wt_pf_save_settings_pro',
...
var wt_pf_settings_custom_fields=
{
	Set:function(prnt_selector)
	{ 
		var wf_prnt_obj=jQuery(prnt_selector);
		wf_prnt_obj.find('.wt_pf_settings_custom_fields_form').submit(function(e){
			e.preventDefault();
			var data=jQuery(this).serialize();
			...
			jQuery.ajax({
				...
				type:'POST',
				dataType:'json',
				data:data+'&action=wt_pf_save_settings_pro_custom_fields',
				success:function(data)
				{
					spinner.css({'visibility':'hidden'});
					submit_btn.css({'opacity':'1','cursor':'pointer'}).prop('disabled',false);
...
var wt_pf_form_toggler=
{
	...
	getElms:function(elm,trgt,vl,checkbox)
	{
		...
		return jQuery('[wf_frm_tgl-id="'+trgt+'"]').filter(function(){
				if(jQuery(this).attr('wf_frm_tgl-val')==vl ||  jQuery(this).attr('wf_frm_tgl-val').includes(vl))
				{
					if(checkbox)
					{
						if(elm.is(':checked'))
...
jQuery(document).ready(function(){
	...
	wt_field_group.Set();
        ...
        wt_pf_settings_custom_fields.Set('#'+wt_pf_basic_params.plugin_id);
});
