...
class Webtoffee_Product_Feed_Shipping {
			...
			foreach ($shipping_zones as $zone) {
				...
				foreach ($locations as $zone_type) {
					...
					$shipping_country_check = apply_filters( 'wt_feed_shipping_country_check', $shipping_country_check );
                                        ...
                                        $shipping['country'] = isset($shipping['country']) ? $shipping['country'] : '';
					if ( 0 === $shipping_country_check && $shipping['country'] !== $this->form_data['post_type_form_data']['wt_pf_export_catalog_country'] ) {
						unset($shipping);
						continue 2;
					}						
	...
	public function get_shipping_cost( $shipping, $method ) {
		...
		$shipping_cost = 0;
                ...
                $wt_feed_is_purchasable = apply_filters('wt_product_feed_product_is_purchasable', $this->product->is_purchasable() );
                if( !$wt_feed_is_purchasable || !$this->product->is_in_stock() || $this->product->is_virtual() || $this->product->is_downloadable() ){
                    return $shipping_cost;
                }   
                
                if(class_exists('Dokan_Pro')){
                    if( null === dokan_pro()->digital_product ){
                        return $shipping_cost;
                    }
                    // Dokan shipping - As this is your own product, the 'Add to Cart' button has been removed. Please visit as a guest to view it.
                    // Changing the user ID to a different number to bail post author comparison early
                    add_filter('dokan_is_product_author', function($current_usr_id){return $current_usr_id+1;} );
                } 
