	...
	class Wt_Pf_Sh 
	{
		...
		*/
		...
		{		
                        ...
                        $sanitize_title_with_dashes_arr = array('wt_pf_inc_exc_category', 'wt_pf_export_post_author', 'wt_pf_inc_exc_brand', 'wt_pf_inc_exc_tag' );
			$static_form_field_elements = array( 'wt_pf_export_post_language', 'wt_pf_export_post_currency', 'wt_pf_exclude_products', 'wt_pf_product_types', 'wt_pf_adv_filter_if[]', 'wt_pf_adv_filter_condition[]', 'wt_pf_adv_filter_val[]', 'wt_pf_adv_filter_then[]' );
		...
		public static function verify_nonce($plugin_id, $nonce_id = '')
		{
    		...
    		if (!$nonce || !(wp_verify_nonce($nonce, $nonce_id))) { //verifying nonce
				if (defined('DOING_AJAX') && DOING_AJAX) {
					wp_send_json_error([
						'status' => 0,
						'msg' => __('Access denied.', 'webtoffee-product-feed-pro')
					], 403);
				} else {
					wp_die(__('Access denied.', 'webtoffee-product-feed-pro'), '', ['response' => 403]);
				}
            	exit;
			}
		    return true;
