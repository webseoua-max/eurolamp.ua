 ...
 * @subpackage Webtoffee_Product_Feed_Sync_Pro/Product
 */
if ( !defined( 'ABSPATH' ) ) {
	exit;
}
	...
	class WT_Facebook_Catalog_Product {
		...
		public function add_sale_price( $product, $product_data ) {
			...
			if ( !is_numeric( $sale_price ) ) {
				...
				$sale_end	 = ( $dates_to	 = get_post_meta( $product->get_id(), '_sale_price_dates_to', true ) ) ? date_i18n( 'Y-m-d', $dates_to ) . 'T23:59+00:00' : '2032-01-30T23:59+00:00';
				...
				$sale_start	 = ( $dates_from	 = get_post_meta( $product->get_id(), '_sale_price_dates_from', true ) ) ? date_i18n( 'Y-m-d', $dates_from ) . 'T00:00+00:00' : '1970-01-29T00:00+00:00';
				// Sale end date is some years from now
				$sale_end	 = ( $dates_to	 = get_post_meta( $product->get_id(), '_sale_price_dates_to', true ) ) ? date_i18n( 'Y-m-d', $dates_to ) . 'T23:59+00:00' : '2032-01-30T23:59+00:00';
		...
		public function prepare_product( $product ) {
			...
			if(class_exists('Alg_WC_Global_Shop_Discount')){
                ...
                $price = $core->add_global_shop_discount($price, $product, 'sale');
				$product_data[ 'sale_price' ] = $price . ' ' . get_woocommerce_currency();
				if($price){
					$regular_price = get_post_meta($product->get_id(), '_regular_price', true);
					$product_data['price'] = $regular_price . ' ' . get_woocommerce_currency();
				}
            }
		...
		public static function get_product_categories( $product_id ) {
			...
			foreach ( $category_path as $category ) {
				...
				if ( $fb_category_id ) {
					$fb_category			 = WT_Fb_Catalog_Manager_Pro_Settings::get_fb_categories( $fb_category_id );
		...
		public function process_item_update( $product_id ) {
			...
			$product = wc_get_product( $product_id );
                        ...
                        // Do not procceed if product object not exist.
