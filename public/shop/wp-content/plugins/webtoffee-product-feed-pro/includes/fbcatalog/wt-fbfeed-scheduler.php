 ...
 */
function wt_schedule_midnight_sync_pro() {
...
//add_action( 'init', 'wt_schedule_midnight_sync_pro' );

/**
 * A callback to run when the 'eg_midnight_log' scheduled action is run.
 */
function wt_fbfeed_midnight_sync_process_pro() {
	error_log( 'It is just after midnight on ' . date( 'Y-m-d' ) );
	$fb_feed_settings = new WT_Fb_Catalog_Manager_Pro_Settings();
	$fb_feed_settings->wt_fbfeed_process_upload();
}

//add_action( 'wt_fbfeed_midnight_sync', 'wt_fbfeed_midnight_sync_process_pro' );
