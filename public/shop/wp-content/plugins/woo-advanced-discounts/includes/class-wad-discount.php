...
class WAD_Discount {
	...
	public function get_sale_price( $sale_price, $product, $include_quantity_based_pricing = true ) {
		...
		if ( 
			($include_quantity_based_pricing && ( is_cart() || is_checkout() || did_action( 'woocommerce_before_mini_cart_contents' ) ))
			||
			// Calculate QBP for the PayPal PPC plugin when it's activated.
			( isset( $_GET['wc-ajax'] ) && $_GET['wc-ajax'] == "ppc-create-order"  )
		) {
