 ...
 * WOE_HTMLPurifier_Lexer_DirectLex,and is the default choice for PHP 5.
 *
...
class WOE_HTMLPurifier_Lexer_DOMLex extends WOE_HTMLPurifier_Lexer
{

    /**
     * @type WOE_HTMLPurifier_TokenFactory
     */
    private $factory;
    ...
    public function __construct()
    {
        // setup the factory
        parent::__construct();
        $this->factory = new WOE_HTMLPurifier_TokenFactory();
    }
     ...
     * @param WOE_HTMLPurifier_Token[] $tokens   Array-list of already tokenized tokens.
     */
    protected function tokenizeDOM($node, &$tokens, $config)
    {
        $level = 0;
        $nodes = array($level => new WOE_HTMLPurifier_Queue(array($node)));
        $closingNodes = array();
        do {
            while (!$nodes[$level]->isEmpty()) {
                ...
                if ($node->childNodes && $node->childNodes->length) {
                    $level++;
                    $nodes[$level] = new WOE_HTMLPurifier_Queue();
    ...
    protected function createStartNode($node, &$tokens, $collect, $config)
    {
        ...
        if ($node->nodeType === XML_TEXT_NODE) {
            ...
            if ($last instanceof WOE_HTMLPurifier_Token_Start && ($last->name == 'script' || $last->name == 'style')) {
                $new_data = trim($data);
