...
class Migrations {
	...
	public function check_migrations() {
		if ( $this->is_migration_error() && isset( $_GET['wbcr_factory_fix_migration_error'] ) ) {
			if ( isset( $_GET['nonce'] ) && ! empty( $_GET['nonce'] ) && wp_verify_nonce( $_GET['nonce'], 'wbcr_fix_migration_error' ) ) {
		...
		if ( $this->is_debug() && isset( $_GET['wbcr_factory_test_migration'] ) ) {
			if ( isset( $_GET['nonce'] ) && ! empty( $_GET['nonce'] ) && wp_verify_nonce( $_GET['nonce'], 'wbcr_migration' ) ) {
				$this->make_migration();
		...
		$fix_migration_error_url = esc_url(
			add_query_arg(
				array(
					...
					'nonce' => wp_create_nonce( 'wbcr_fix_migration_error' ),
		 		),
			)
		);
		...
		$migrate_url = esc_url(
			add_query_arg(
				array(
					...
					'nonce' => wp_create_nonce( 'wbcr_migration' )
				)
			)
		);
