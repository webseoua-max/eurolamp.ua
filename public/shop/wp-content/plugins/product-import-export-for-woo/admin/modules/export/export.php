...
if(!class_exists('Wt_Import_Export_For_Woo_Product_Basic_Export')){
class Wt_Import_Export_For_Woo_Product_Basic_Export
{
	public $module_id='';
	...
	private static $user_post_types = array('user');
	...
	public function __construct()
	{
		...
		$this->allowed_export_file_type=array(
			'csv'=>__('CSV', 'product-import-export-for-woo'),
			'xml'=>__('XML', 'product-import-export-for-woo')
		);
		...
		(
			'post_type'=>array(
				'title'=>__('Select a post type', 'product-import-export-for-woo'),
				'description'=>__('Export and download the data for the respective post type into a CSV file. As per the selected post type, we can use this exported CSV file to import data to your site.', 'product-import-export-for-woo'),
			),
			'method_export'=>array(
				'title'=>__('Select an export method', 'product-import-export-for-woo'),
				'description'=>__('Choose from the options below to continue with your export: quick export from DB, based on a pre-saved template or a new export with advanced options.', 'product-import-export-for-woo'),
			),
			'filter'=>array(
				'title'=>__('Filter data', 'product-import-export-for-woo'),
				'description'=>__('Filter data that needs to be exported as per the below criteria.', 'product-import-export-for-woo'),
			), 
			'mapping'=>array(
				'title'=>__('Map and reorder export columns', 'product-import-export-for-woo'),
				'description'=>__('The default export column names can be edited from the screen below, if required. If you have chosen a pre-saved template you can see the preferred names and choices that were last saved. You may also drag the columns accordingly to reorder them within the output file.', 'product-import-export-for-woo'),
			),
			'advanced'=>array(
				'title'=>__('Advanced options/Batch export', 'product-import-export-for-woo'),
				'description'=>__('You can save the template file for future exports or proceed with the export.', 'product-import-export-for-woo'),
			),
		);
		...
		$this->export_methods=array(
			'quick'=>array('title'=>__('Quick export', 'product-import-export-for-woo'), 'description'=> __('Exports all the basic fields.', 'product-import-export-for-woo')),
			'template'=>array('title'=>__('Pre-saved template', 'product-import-export-for-woo'), 'description'=> __('Exports data as per the specifications(filters,selective column,mapping etc) from the previously saved file.', 'product-import-export-for-woo')),
			'new'=>array('title'=>__('Advanced export', 'product-import-export-for-woo'), 'description'=> __('Exports data after a detailed process of filtration, column selection and advanced options. The configured settings can be saved as a template for future exports.', 'product-import-export-for-woo')),
		);
	...
	{	
		...
		$this->use_bom = (bool)Wt_Import_Export_For_Woo_Product_Basic_Common_Helper::get_advanced_settings('include_bom');
	}
	...
	*/
	public function advanced_setting_fields($fields)
	{
		...
		$fields['default_export_method']=array(
			'label'=>__("Default Export method", 'product-import-export-for-woo'),
			'type'=>'select',
			'sele_vals'=>$export_methods,
            'value' =>'new',
			'field_name'=>'default_export_method',
			'field_group'=>'advanced_field',
			'help_text'=>__('Select the default method of export.', 'product-import-export-for-woo'),
		);
		
		$fields['include_bom'] = array(
			'label'=>__( "Include BOM in export file", 'product-import-export-for-woo' ),
			'value'=>1,
			'checkbox_fields' => array( 1 => __( 'Enable', 'product-import-export-for-woo' ) ),
			'type'=>'checkbox',
			'field_name'=>'include_bom',
			'field_group'=>'advanced_field',
			'help_text'=>__( "The BOM will help some programs like Microsoft Excel read your export file if it includes non-English characters.", 'product-import-export-for-woo' ),
		);
		
		$fields['default_export_batch']=array(
			'label'=>__("Default Export batch count", 'product-import-export-for-woo'),
			'type'=>'number',
                        'value' =>30,
		...
		$fields['enable_chatgpt'] = array(
			'label'=>__( "Enable ChatGPT", 'product-import-export-for-woo' ),
			'value'=>0,
			'checkbox_fields' => array( 1 => __( 'Enable', 'product-import-export-for-woo' ) ),
			'type'=>'checkbox',
			'field_name'=>'enable_chatgpt',
			'field_group'=>'advanced_field',
			'help_text'=>__( 'Automatically generate product descriptions from product titles using ChatGPT API for products without descriptions in the importing CSV.', 'product-import-export-for-woo' ),
                    	'form_toggler'=>array(
				'type'=>'parent',
		...
		$fields['chatgpt_api_key'] = array(
                        'label' => __("Chat GPT API key", 'product-import-export-for-woo'),
                        'type' => 'text',
                        'value' => '',
                        'field_name' => 'chatgpt_api_key',
                        'field_group' => 'advanced_field',
                        // translators: %1$s is opening link tag, %2$s is closing link tag
                        'help_text' => sprintf(__('Input the ChatGPT API key to enable the automatic generation of product descriptions. %1$s Where do I get my API Keys? %2$s ', 'product-import-export-for-woo'), '<a href="https://help.openai.com/en/articles/4936850-where-do-i-find-my-secret-api-key" target="_blank">', '</a>'),                        
	...
	{		
		$menu_temp=array(
			$this->module_base=>array(
				'menu',
				__('Export', 'product-import-export-for-woo'),
				__('WebToffee Import Export (Basic)', 'product-import-export-for-woo'),
	...
	*/
	public function admin_settings_page()
	{
		...
		*/
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification handled by admin interface
	...
	*/
	...
	{  
		...
		if(Wt_Iew_Sh::check_write_access(WT_IEW_PLUGIN_ID_BASIC))
		{
			...
			*	Check it is a rerun call
			*/
			// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verification is handled by Wt_Iew_Sh::check_write_access() above
			if(!$this->_process_rerun((isset($_POST['rerun_id']) ? absint($_POST['rerun_id']) : 0)))
			{	
				// phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified above, input is sanitized via Wt_Iew_Sh::sanitize_item()
				$this->export_method=(isset($_POST['export_method']) ? Wt_Iew_Sh::sanitize_item(wp_unslash($_POST['export_method']), 'text') : '');
				// phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified above, input is sanitized via Wt_Iew_Sh::check_write_access() above
				$this->to_export=(isset($_POST['to_export']) ? Wt_Iew_Sh::sanitize_item(wp_unslash($_POST['to_export']), 'text') : '');
				// phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified above, input is sanitized via Wt_Iew_Sh::sanitize_item()
				$this->selected_template=(isset($_POST['selected_template']) ? Wt_Iew_Sh::sanitize_item(wp_unslash($_POST['selected_template']), 'int') : 0);
			}		
			...
			if (!in_array($this->to_export, self::$handled_post_types)) {
				 ...
				 */
				if ( in_array( $this->to_export, self::$order_post_types ) ) {
					if ( defined('WT_O_IEW_VERSION') && version_compare( WT_O_IEW_VERSION, '2.7.0', '<' ) ) {
						$should_return = false;
					}
				} elseif ( in_array( $this->to_export, self::$user_post_types ) ) {
					if ( defined('WT_U_IEW_VERSION') && version_compare( WT_U_IEW_VERSION, '2.7.0', '<' ) ) {
						$should_return = false;
					}
				}
			...
			$ajax_obj=new Wt_Import_Export_For_Woo_Product_Basic_Export_Ajax($this, $this->to_export, $this->steps, $this->export_method, $this->selected_template, $this->rerun_id);
			...
			$export_action=Wt_Iew_Sh::sanitize_item(isset($_POST['export_action']) ? wp_unslash($_POST['export_action']) : '', 'text');
			// phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.ValidatedSanitizedInput.InputNotValidated,WordPress.Security.ValidatedSanitizedInput.MissingUnslash,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Nonce verified above, input is sanitized via Wt_Iew_Sh::sanitize_item()
    ...
    public function get_advanced_screen_fields($advanced_form_data)
	{
		...
		$advanced_screen_fields=array(
			'file_name'=>array(
				...
				'help_text'=>__('Specify a filename for the exported file. If left blank, the system generates the name automatically.', 'product-import-export-for-woo'),
			...
			'delimiter'=>array(
				...
				'help_text'=>__( 'Separator for differentiating the columns in the CSV file. Assumes comma by default.', 'product-import-export-for-woo' ),
	...
	*/
	...
	protected function enqueue_assets()
	{
        if ( Wt_Import_Export_For_Woo_Product_Basic_Common_Helper::wt_is_screen_allowed() ) {
			wp_enqueue_script($this->module_id, plugin_dir_url(__FILE__).'assets/js/main.js', array('jquery', 'jquery-ui-sortable', 'jquery-ui-datepicker'), WT_P_IEW_VERSION, true);
			...
			$params=array(
				...
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- GET parameter for UI state, nonce not required
				...
				'msgs'=>array(
					'choosed_template'=>__('Choosed template:', 'product-import-export-for-woo'),
					'choose_export_method'=>__('Please select an export method.', 'product-import-export-for-woo'),
					'choose_template'=>__('Please select an export template.', 'product-import-export-for-woo'),
					'step'=>__('Step', 'product-import-export-for-woo'),
					'choose_ftp_profile'=>__('Please select an FTP profile.', 'product-import-export-for-woo'),
	...
	*/
	public function process_upload($step, $export_id, $to_export)
	{
		...
		{
			$remote_adapter=Wt_Import_Export_For_Woo_Product_Basic::get_remote_adapters('export', $file_into);
			...
			$out['msg'] = (isset($upload_data['msg']) ? $upload_data['msg'] : __('Error', 'product-import-export-for-woo'));
		...
		{
			$out['msg']=__('Finished', 'product-import-export-for-woo');
	...
	*/
	public function process_action($form_data, $step, $to_process, $file_name='', $export_id=0, $offset=0)
	{
		...
		if($file_path===false)
		{
            ...
            $out['msg']= __( $msg, 'product-import-export-for-woo');
            return $out;
		}
		...
		if($is_last_offset) //finished
		{
			...
			if('local' != $file_into) /* file not save to local. Initiate the choosed remote profile */
			{
				...
				// translators: %s is the upload destination name
				$out['msg']=sprintf(__('Uploading to %s', 'product-import-export-for-woo'), $file_into);
			}else
			{
				$out['file_url']=html_entity_decode($this->get_file_url($file_name));
		...
		}else
		{
			...
			// translators: %1$d is current offset, %2$d is total records
			$out['msg']=sprintf(__('Exporting...(%1$d out of %2$d)', 'product-import-export-for-woo'), $new_offset, $total_records);
		}
		return $out;
	}
	...
	*/
	...
	{
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verification is handled by check_write_access()
		if(isset($_GET['wt_iew_export_download'])) {

			if(Wt_Iew_Sh::check_write_access(WT_IEW_PLUGIN_ID_BASIC)) { /* check nonce and role */
			
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.MissingUnslash -- Nonce verified above
				...
				if($file_name != "") {
					...
					if(isset($this->allowed_export_file_type[$file_ext]) || $file_ext == 'zip') { /* Only allowed files. Zip file in image export */
						...
						if($file_path && $export_dir && strpos($file_path, $export_dir) === 0 && file_exists($file_path) && is_file($file_path)) { /* check existence of file and verify path */
						    ...
						    // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- Direct filesystem operation needed for file download
						    $handle=@fopen($file_path, 'rb');
						    while(!feof($handle))
						    {
						        // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fread -- Direct filesystem operation needed for file download
						        $buffer = fread($handle, $chunk_size);
						        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Binary file output for download, escaping not applicable
						        echo $buffer;
						        ob_flush();
						        flush();
						    }
						    // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- Direct filesystem operation needed for file download
						    fclose($handle);
						    exit();
						}
					}
...
Wt_Import_Export_For_Woo_Product_Basic::$loaded_modules['export']=new Wt_Import_Export_For_Woo_Product_Basic_Export();