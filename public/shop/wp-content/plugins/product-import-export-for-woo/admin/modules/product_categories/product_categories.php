...
if(!class_exists('Wt_Import_Export_For_Woo_Product_Basic_Product_Categories')){
class Wt_Import_Export_For_Woo_Product_Basic_Product_Categories {
    ...
    */
    public function importer_steps($steps, $base)
    {
        ...
        {
            $steps['advanced']['description']=__('Use advanced options from below to decide updates to existing categories, batch import count. You can also save the template file for future imports.', 'product-import-export-for-woo');
        }
        return $steps;
    }
    ...
    public static function get_categories_sort_columns() {
        $sort_columns = array(
            'id' => __('Category ID', 'product-import-export-for-woo'),
            'name' => __('Category name', 'product-import-export-for-woo'),
            'slug' => __('Category slug', 'product-import-export-for-woo'),
        );
...
//        $colunm = array_map(function($vl){ return array('title'=>$vl, 'description'=>$vl); }, $arr); 
    ...
    public function importer_alter_advanced_fields($fields, $base, $advanced_form_data) {
        ...
        $out['merge'] = array(
            'label' => __("If the category exists in the store", 'product-import-export-for-woo'),
            'type' => 'radio',
            'radio_fields' => array(                
            ...
            'value' => '0',
            'field_name' => 'merge',
            'help_text' => __('Categories are matched by their ID/slugs.', 'product-import-export-for-woo'),
            'help_text_conditional'=>array(
                array(
                    'help_text'=> __('Retains the categories in the store as is and skips the matching category from the input file.', 'product-import-export-for-woo'),
                    'condition'=>array(
                        array('field'=>'wt_iew_merge', 'value'=>0)
                    )
                ),
                array(
                    'help_text'=> __('Update category as per data from the input file', 'product-import-export-for-woo'),
                    'condition'=>array(
...
new Wt_Import_Export_For_Woo_Product_Basic_Product_Categories();
