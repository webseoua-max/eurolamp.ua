 ...
 */
class XFGMC_Feeds_List_Table extends WP_List_Table {
	...
	public function display_html_form() {
		...
		echo '<form method="get"><input type="hidden" name="xfgmc_form_id" value="xfgmc_wp_list_table" />';
		wp_nonce_field( 'xfgmc_nonce_action_f', 'xfgmc_nonce_field_f' );
		...
		if ( ! empty( $settings_arr ) ) {
			...
			if ( ! empty( $feed_ids_arr ) ) {
				for ( $i = 0; $i < count( $feed_ids_arr ); $i++ ) {
					...
					// Назначение фида
					if ( isset( $settings_arr[ $feed_id_str ]['xfgmc_feed_assignment'] ) ) {
						$feed_assignment = $settings_arr[ $feed_id_str ]['xfgmc_feed_assignment'];
					} else {
						$feed_assignment = '';
					}
					...
					if ( isset( $settings_arr[ $feed_id_str ]['xfgmc_run_cron'] ) ) {
						...
						if ( false === $cron_info ) {
							$cron_info = wp_get_scheduled_event( 'xfgmc_cron_start_feed_creation', [ $feed_id_str ] );
							if ( false === $cron_info ) {
					...
					if ( isset( $settings_arr[ $feed_id_str ]['xfgmc_count_products_in_feed'] ) ) {
						$offes_total = (int) $settings_arr[ $feed_id_str ]['xfgmc_count_products_in_feed'] - (int) 1;
						if ( $offes_total < 0 ) {
							$offes_total = 0;
						}
					...
					// if ( isset( $settings_arr[ $feed_id_str ]['xfgmc_critical_errors'] )
					// 	&& ! empty( $settings_arr[ $feed_id_str ]['xfgmc_critical_errors'] )
					// ) {
					// 	$text_column_feed_summary .= sprintf(
					// 		'<strong>%s:</strong> %s',
