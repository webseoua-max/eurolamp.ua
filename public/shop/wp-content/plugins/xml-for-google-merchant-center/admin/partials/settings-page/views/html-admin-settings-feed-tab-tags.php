...
$attr_arr = $plugin_date->get_options( [ 'xfgmc_xml_rules' ] );
...
for ( $i = 0; $i < count( $attr_arr ); $i++ ) {
	...
	if ( ( isset( $rules_arr[ $xml_rules ] ) && in_array( $tag_name, $rules_arr[ $xml_rules ] ) )
...
for ( $i = 0; $i < count( $attr_arr ); $i++ ) {
	...
	if ( ! isset( $attr_arr[ $i + 1 ]['data']['tag_name'] )
		...
		<div class="xfgmc-postbox postbox closed">
			<div class="postbox-header">
				<h2 class="hndle ui-sortable-handle"><?php echo wp_kses( $html_header, XFGMC_ALLOWED_HTML_ARR ); ?></h2>
			</div>
			<div class="inside" style="padding: 0 !important;">
				<table class="form-table" role="presentation">
					<tbody>
						<?php echo wp_kses( $html_body, XFGMC_ALLOWED_HTML_ARR ); ?>
					</tbody>
				</table>
			</div>
		</div>
