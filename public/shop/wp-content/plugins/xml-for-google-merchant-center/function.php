<?php
if ( ! function_exists( 'xfgmc_get_html_options' ) ) {
	/**
	 * Get `option` tags for HTML form.
	 * 
	 * @since 4.0.0 (02-06-2025)
	 * 
	 ...
	 * @return string
	 */
	function xfgmc_get_html_options( $opt_value, $params_arr = [], $multiple = false ) {
...
if ( ! function_exists( 'xfgmc_get_html_options_for_select2' ) ) {
	/**
	 * @since 1.0.4
	 ...
	 */
	function xfgmc_get_html_options_for_select2( $opt_value_arr ) {
...
if ( ! function_exists( 'xfgmc_get_dir_files' ) ) {
	/**
	 ...
	 */
	function xfgmc_get_dir_files( $dir, $recursive = true, $include_folders = false ) {
		...
		// $files = xfgmc_get_dir_files( plugin_dir_path( dirname( __FILE__ ) ) . 'includes/feeds/' );
...
if ( ! function_exists( 'xfgmc_strip_tags' ) ) {
	/**
	 ...
	 * @return string
	 */
	function xfgmc_strip_tags( $tag_value, $enable_tags = '' ) {
...
if ( ! function_exists( 'get_nested_tag' ) ) {
	...
	function get_nested_tag( $name_wrapper_tag, $name_nested_tag, $tag_value, $result_xml = '' ) {
		...
		if ( count( $elements ) > 1 ) {
			$result_xml .= new XFGMC_Get_Open_Tag( $name_wrapper_tag );
			foreach ( $elements as $element ) {
				$result_xml .= new XFGMC_Get_Paired_Tag( $name_nested_tag, $element );
			}
			$result_xml .= new XFGMC_Get_Closed_Tag( $name_wrapper_tag );
		} else if ( count( $elements ) === (int) 1 ) {
			$result_xml .= new XFGMC_Get_Paired_Tag( $name_wrapper_tag, $elements[0] );
		}
		return $result_xml;
...
if ( ! function_exists( 'xfgmc_replace_decode' ) ) {
	/**
	 * @since 3.3.16
	 *
	 * @return string
	 */
	function xfgmc_replace_decode( $string, $feed_id = '1' ) {

		$string = str_replace( "+", 'xfgmc', $string );
		$string = urldecode( $string );
		$string = str_replace( "xfgmc", '+', $string );
		$string = apply_filters( 'xfgmc_replace_decode_filter', $string, $feed_id );
		return $string;

	}
}

if ( ! function_exists( 'xfgmc_replace_domain' ) ) {
	/**
...
if ( ! function_exists( 'xfgmc_remove_special_characters' ) ) {
	/**
	 ...
	 * @return string
	 */
	function xfgmc_remove_special_characters( $result_xml ) {
...
if ( ! function_exists( 'xfgmc_global_set_woocommerce_currency' ) ) {
	/**
	 * Set currency from FOX - Currency Switcher Professional for WooCommerce.
	 * 
	 * @since 4.0.7
	 *
	 * @param string $feed_id
	 ...
	 * @return void
	 */
	function xfgmc_global_set_woocommerce_currency( $feed_id ) {
...
if ( ! function_exists( 'xfgmc_global_rest_woocommerce_currency' ) ) {
	/**
	 * Rest currency from FOX - Currency Switcher Professional for WooCommerce.
	 * 
	 * @since 4.0.7
	 *
	 * @return void
	 */
	function xfgmc_global_rest_woocommerce_currency() {
