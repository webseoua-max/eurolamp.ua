...
trait XFGMC_T_Simple_Get_Unit_Pricing_Measure {
	...
	public function get_unit_pricing_measure( $tag_name = 'g:unit_pricing_measure', $result_xml = '' ) {
		...
		if ( $unit_pricing_measure === 'disabled' ) {
			...
			$tag_value = $this->get_simple_product_post_meta( 'unit_pricing_measure' );
			if ( class_exists( 'WooCommerce_Germanized' ) ) {
				...
				if ( ! empty( wc_gzd_get_gzd_product( $this->get_product()->get_id() )->get_unit_product() ) ) {
					$tag_value = wc_gzd_get_gzd_product( $this->get_product()->get_id() )->get_unit_product() . $unit_germanized;
				}
			}
