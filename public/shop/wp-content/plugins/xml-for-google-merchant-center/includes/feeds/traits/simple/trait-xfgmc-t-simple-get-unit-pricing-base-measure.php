 ...
 *             functions:   
 */
trait XFGMC_T_Simple_Get_Unit_Pricing_Base_Measure {
	...
	public function get_unit_pricing_base_measure( $tag_name = 'g:unit_pricing_base_measure', $result_xml = '' ) {
		...
		$tag_value = '';
		$tag_value = $this->get_simple_product_post_meta( 'unit_pricing_base_measure' );
		if ( class_exists( 'WooCommerce_Germanized' ) ) {
			...
			if ( ! empty( wc_gzd_get_gzd_product( $this->get_product()->get_id() )->get_unit_base() ) ) {
				$tag_value = trim( sprintf( '%s %s',
					wc_gzd_get_gzd_product( $this->get_product()->get_id() )->get_unit_base(),
					$unit_germanized
				) );
			}
		}
		...
		$result_xml = apply_filters(
			'xgfmc_f_simple_tag_unit_pricing_base_measure',
			$result_xml,
			[ 
