 ...
 *             traits:      XFGMC_T_Get_Post_Id
 *                          XFGMC_T_Get_Feed_Id;
 *                          XFGMC_T_Get_Product
 *                          XFGMC_T_Get_Skip_Reasons_Arr
 */
class XFGMC_Get_Unit {

	use XFGMC_T_Get_Post_Id;
	use XFGMC_T_Get_Feed_Id;
	use XFGMC_T_Get_Product;
	use XFGMC_T_Get_Skip_Reasons_Arr;
	...
	public function __construct( $post_id, $feed_id, $date_successful_feed_update = 0, $date_save_set = 0 ) {
		...
		do_action( 'xfgmc_before_wc_get_product', $args_arr );
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			for ( $i = 0; $i < $variation_count; $i++ ) {
				...
				$stop_flag = apply_filters(
					'xfgmc_f_after_variable_offer_stop_flag',
					$stop_flag,
					[ 
						'i' => $i,
						'variation_count' => $variation_count,
	 ...
	 * Set result.
	 * 
	 * @param XFGMC_Get_Unit_Offer $offer_obj
	 * 
	 * @return bool
	 */
