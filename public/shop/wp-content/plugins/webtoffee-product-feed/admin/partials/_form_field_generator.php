...
foreach($form_fields as $key=>$value)
{
	...
	*/
	...
	if($tr_html=="")
	{
		...
		<tr <?php echo wp_kses_post( $tr_id.$field_group_attr );?> class="<?php echo esc_attr( $tr_class );?>">
			<td colspan="3" class="wt_pf_field_group">
				<div class="wt_pf_field_group_hd">
					...
					<div class="wt_pf_field_group_toggle_btn" data-id="<?php echo isset($value['group_id']) ? esc_attr( $value['group_id'] ) : ''; ?>" data-visibility="<?php echo esc_attr( $visibility ); ?>"><span class="dashicons dashicons-arrow-<?php echo ($visibility==1 ? 'down' : 'right'); ?>"></span></div>
				</div>
		...
		{		
		?>		
		<tr <?php echo wp_kses_post( $tr_id.$field_group_attr );?> class="<?php echo esc_attr( $tr_class );?>"  <?php echo wp_kses_post( $form_toggler_child ); ?>>
			<?php
			if(!$merge_left) /* if merge left not enabled */
			{
			?>
				<th class="<?php echo esc_attr( $td_class1 );?>">
					<label><?php echo isset($value['label']) ? esc_html( $value['label'] ) : ''; ?></label>
				</th>
			<?php
			}
			?>
			<td <?php echo wp_kses_post( $colspan_attr );?> class="<?php echo esc_attr( $td_class2 );?>">
				...
				if($field_html=='')
				{
					...
					{
						?>
						<textarea placeholder="<?php echo esc_attr( $placeholder ); ?>" <?php echo wp_kses_post( $html_id ); ?> <?php echo wp_kses_post( $css_attr ); ?> name="<?php echo wp_kses_post( $form_data_key ); ?>" <?php echo wp_kses_post( $attr ); ?> ><?php echo wp_kses_post( $vl ); ?></textarea>
						<?php
					}
					...
					echo wp_kses( $after_form_field, $allowed_markup_tags );
				}else
				{
				...
				{
					?>
					<span class="wt-pfd_form_help"><?php echo wp_kses_post( $help_text );?></span>
					<?php
				}
				?>
			</td>
